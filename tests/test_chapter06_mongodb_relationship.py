...
from chapter06.mongodb_relationship.app import app
from chapter06.mongodb_relationship.database import get_database
from chapter06.mongodb_relationship.models import Comment, Post

motor_client = AsyncIOMotorClient(
    os.getenv("MONGODB_CONNECTION_STRING", "mongodb://localhost:27017")
)
database_test = motor_client["chapter06_mongo_relationship_test"]
initial_posts = [
    Post(
        title="Post 1",
        content="Content 1",
        comments=[
            Comment(content="Post 1 Comment 1"),
            Comment(content="Post 1 Comment 2"),
            Comment(content="Post 1 Comment 3"),
        ],
    ),
...
async def initialize_database():
    ...
    await motor_client.drop_database("chapter06_mongo_relationship_test")
...
@pytest.mark.asyncio
class TestChapter06MongoDBRelationship:
